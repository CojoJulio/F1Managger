...
const CircuitMap = ({ drivers, selectedCar }) => {
  ...
  const getDriverPosition = (progress) => {
    ...
    // Punto un poco más adelante para calcular dirección
    const delta = 2;
  ...
  return (
    <div className="w-full h-full bg-black">
      <svg viewBox="-120 -20 800 350" className="w-full h-full">
        ...
        {/* Cars */}
        {drivers.map((driver) => {
          const { x, y, angle } = getDriverPosition(driver.state.lapProgress);
          ...
          return (
            ...
            >
              <circle
                ...
                cy="0"
                r="10"
                fill={driver.team.team_color || 'gray'}
                stroke="#fff"
                strokeWidth={selectedCar?.id === driver.id ? 7 : 1}
                rx="2"
              />
            </g>
          );
